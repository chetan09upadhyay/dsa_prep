...
// Define theme-based edge colors
const edgeStyles = {
  light: {
    default: "#000000", // Black for light mode
    highlighted: "#2563eb", // Blue for highlighted edges in light mode
  },
  dark: {
    default: "#ffffff", // White for dark mode
    highlighted: "#60a5fa", // Lighter blue for highlighted edges in dark mode
  },
};
...
export function HeapDisplay({ heap, highlightedNodes }: HeapDisplayProps) {
  ...
  // Update nodes and edges when heap, highlightedNodes, or theme changes
  useEffect(() => {
    if (!heap) {
      setNodes([]);
      setEdges([]);
    ...
    // Determine effective theme: treat "system" as resolvedTheme
        ...
        newEdges.push({
          ...
          style: {
            stroke: isHighlighted
              ? edgeStyles[isDarkMode ? "dark" : "light"].highlighted
              : edgeStyles[isDarkMode ? "dark" : "light"].default,
            strokeWidth: 1.5,
    ...
    setEdges(newEdges);
    fitView();
  }, [heap, highlightedNodes, theme, resolvedTheme, setNodes, setEdges, fitView]);
